...
class TestSchwabPositions(unittest.TestCase):
    def setUp(self) -> None:
        self.positions = list(
            schwab.SchwabAccount(
                positions=Path("tests/schwab_positions.CSV")
            ).positions()
        )
    ...
    def test_tBill(self) -> None:
        self.assertEqual(self.positions[0].instrument, Bond("193845XM2", Currency.USD))
        self.assertEqual(self.positions[0].quantity, 10000)
        self.assertEqual(
            self.positions[0].costBasis, helpers.cashUSD(Decimal("9956.80"))
        )

    def test_bnd(self) -> None:
        self.assertEqual(self.positions[1].instrument, Stock("BND", Currency.USD))
        self.assertEqual(self.positions[1].quantity, Decimal("36.8179"))
        self.assertEqual(
            self.positions[1].costBasis, helpers.cashUSD(Decimal("1801.19"))
        )

    def test_uvxy(self) -> None:
        self.assertEqual(self.positions[2].instrument, Stock("UVXY", Currency.USD))
        self.assertEqual(self.positions[2].quantity, Decimal("0"))
        self.assertEqual(self.positions[2].costBasis, helpers.cashUSD(Decimal("0")))

    def test_vti(self) -> None:
        self.assertEqual(self.positions[3].instrument, Stock("VTI", Currency.USD))
        self.assertEqual(self.positions[3].quantity, Decimal("48.2304"))
        self.assertEqual(
            self.positions[3].costBasis, helpers.cashUSD(Decimal("3283.04"))
        )
...
class TestSchwabTransactions(unittest.TestCase):
    def setUp(self) -> None:
        self.activity = list(
            schwab.SchwabAccount(
                transactions=Path("tests/schwab_transactions.CSV")
            ).activity()
        )
    ...
    def test_buyBond(self) -> None:
        ...
        self.assertEqual(
            ...
            Trade(
                ...
                amount=Cash(currency=Currency.USD, quantity=Decimal("-9956.80")),
    ...
    def test_buyStock(self) -> None:
        ...
        self.assertEqual(
            ...
            Trade(
                ...
                quantity=Decimal("23"),
                amount=Cash(currency=Currency.USD, quantity=Decimal("-4981.11")),
    ...
    def test_shortSaleAndCover(self) -> None:
        ...
        self.assertEqual(
            ...
            Trade(
                ...
                instrument=Stock("HD", Currency.USD),
                quantity=Decimal("6"),
                amount=Cash(currency=Currency.USD, quantity=Decimal("-1033.12")),
...
class TestSchwabBalance(unittest.TestCase):
    ...
    def testUSDBalance(self) -> None:
        self.assertEqual(
            self.balance.cash, {Currency.USD: helpers.cashUSD(Decimal("500.21"))}
        )
